...
def subsample_kraken2(input_path, bs_id_str, mhg, cs, subsampling_fractions, replicate_nr, output_path):
    with open('{}BS18-{}_mhg{}_cs{}.kraken2'.format(input_path, bs_id_str, mhg, cs), 'r') as inf:
        lines = inf.readlines()

    for sf in tqdm(subsampling_fractions):
        line_nr = int(sf * len(lines))
        for r in range(1, replicate_nr + 1):
            with open('{}BS18-{}_mhg{}_cs{}_sf{}_rep{}.kraken2'.format(output_path, bs_id_str, mhg, cs, str(sf).replace('.', ''), r), 'w') as outf:
...
def main():
    ...
    subsample_kraken2(args.kraken_path, bs_id_str, args.mhg, args.cs, subsampling_fractions, replicate_nr, args.subsample_path)
